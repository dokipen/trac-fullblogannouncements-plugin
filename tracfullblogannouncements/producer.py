...
class BlogChangeEvent(AnnouncementEvent):
    def __init__(self, blog_post, category, blog_comment=None):
        ...
        self.remote_addr = self.env.abs_href.blog(blog.name)
...
class BlogChangeProducer(Component):
    ...
    def blog_comment_added(self, postname, number):
        ...
        announcer = AnnouncementSystem(page.env)
        announcer.send(
            BlogChangeEvent(blog_post, 'comment created', blog_comment)
        )
    ...
    def blog_comment_deleted(self, postname, number, fields):
        ...
        announcer = AnnouncementSystem(page.env)
        announcer.send(
            BlogChangeEvent(blog_post, 'comment deleted', fields)
        )
